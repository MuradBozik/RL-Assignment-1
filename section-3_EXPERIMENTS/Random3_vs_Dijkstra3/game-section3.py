...
def plotElo(*argv):
    ...
    ax.set(xlabel='game (n)', ylabel='Elo (mu)', title='Elo change for 100 games 4 x 4')
    ax.legend(['BLUE', 'RED'])
    ax.grid()
    fig.savefig('fig.png')
    plt.show()
    ...
    while experiment_number > 0:
        ...
        while True:
            if (game.turn % 2) == 0:
                ...
                best_value = util.alphaBetaSearchDijkstra(node, 3, -9999999, 9999999, isMaximizer=True)
                move = util.getBestMove(node, best_value)
                game = makeMove(move, game)
                node = util.updateNode(node, game)
