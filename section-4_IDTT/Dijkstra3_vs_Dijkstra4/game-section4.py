...
if __name__ == '__main__':
    ...
    while experiment_number > 0:
        ...
        while True:
            if (game.turn % 2) == 0:
                # Computer 1's turn (Dijkstra 3)
                best_value = util.alphaBetaSearchDijkstra(node, 3, -9999999, 9999999, isMaximizer=False)
                move = util.getBestMove(node, best_value)
                game = makeMove(move, game)
                node = util.updateNode(node, game)
            else:
                # Computer 2's turn (Dijkstra 4)
                boardState = node['board'].copy()
                try:
                    move = TranspositionTable[boardState.tobytes()]
                except KeyError:
                    best_value = util.iterativeDeepening(node, True)
                    move = util.getBestMove(node, best_value)
                    TranspositionTable[boardState.tobytes()] = move
                game = makeMove(move, game)
                node = util.updateNode(node, game)
